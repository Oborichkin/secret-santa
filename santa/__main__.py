...
enroll_handler = ConversationHandler(
    ...
    states={States.ENROLLING: [MessageHandler(filters.TEXT & ~filters.Text("Cancel"), enroll)]},
    fallbacks=[MessageHandler(filters.Text("Cancel"), cancel)],
)

application.add_handlers([start_handler, create_handler, members_handler, begin_handler])
